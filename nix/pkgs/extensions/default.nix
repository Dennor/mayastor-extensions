{
  ...
  versionDrv = import ../../lib/version.nix {inherit sourcer lib stdenv git tag long_ver;};
  ...
  components = {
    ...
    metrics = {
      exporter = rec {
        ...
        metrics_builder = {
          buildType,
          builder,
          cargoBuildFlags ? ["-p metrics-exporter"],
        }:
          builder.build {inherit buildType cargoBuildFlags;};
        metrics_installer = {
          pname,
          src,
        }:
          installer {inherit pname src;};
        io-engine = metrics_installer {
          src =
            if allInOne
            then metrics_builder {inherit buildType builder;}
            else
              metrics_builder {
                inherit buildType builder;
                cargoBuildFlags = ["--bin metrics-exporter-io-engine"];
              };
    ...
    upgrade = rec {
      ...
      job = upgrade_installer {
        src =
          if allInOne
          then upgrade_builder {inherit buildType builder;}
          else
            upgrade_builder {
              inherit buildType builder;
              cargoBuildFlags = ["--bin upgrade-job"];
            };
        pname = "upgrade-job";
      };
    };
      ...
      callhome = obs_installer {
        src =
          ...
          then
            ...
            obs_builder {
              inherit buildType builder;
              cargoBuildFlags = ["--bin obs-callhome"];
            };
        pname = "obs-callhome";
      };
      stats = obs_installer {
        src =
          ...
          then
            obs_builder {
              inherit buildType builder;
              cargoBuildFlags = ["-p call-home-stats"];
            }
          else
            obs_builder {
              inherit buildType builder;
              cargoBuildFlags = ["--bin call-home-stats"];
            };
